...
interface DriveFolder { id: string; name: string; childCount: number }
...
interface Props {
  ...
  msTenantId: string;
  selection: OneDriveSelection | null;
  onSelect: (s: OneDriveSelection) => void;
}
...
export default function OneDriveFolderPicker({
  ...
  const selectCurrentLocation = () => {
    if (breadcrumb.length === 0) {
      onSelect({ folderId: "root", folderPath: "OneDrive (root)" });
    } else {
      ...
      <div className="space-y-3">
        ...
        {!selection && (
          <button type="button" onClick={connect} disabled={loading}
            ...
            {loading ? "Connecting…" : "Browse OneDrive Folders"}
          </button>
        )}
        {error && (
      ...
      {!loading && (
        <div className="px-4 py-3 border-t border-slate-100 bg-slate-50">
          <button type="button" onClick={selectCurrentLocation}
            ...
            Use this location ({breadcrumb.length === 0 ? "OneDrive (root)" : breadcrumb.map((b) => b.name).join(" / ")})
          </button>
        </div>
      )}
    </div>
