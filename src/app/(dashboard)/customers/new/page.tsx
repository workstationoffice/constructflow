...
import {
  ...
  Loader2, AlertCircle, Hash, Home, Truck, Star,
} from "lucide-react";
...
export default function NewCustomerPage() {
  ...
  const [name, setName]               = useState(""); // used only for PERSONAL type
  ...
  const updateAddress = (i: number, field: keyof AddressRow, value: any) => {
    setAddresses((prev) => {
      ...
      // Only one default billing / one default shipping
    ...
    try {
      ...
      const res = await fetch("/api/customers", {
        ...
        body: JSON.stringify({
          ...
          companyName: type === "COMPANY" ? companyName || undefined : undefined,
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {type === "COMPANY" && (
          <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">เลขนิติบุคคล (13 หลัก)</label>
              <div className="flex gap-2">
                <div className="flex-1 relative">
                  <input value={taxId} onChange={(e) => { setTaxId(formatTaxId(e.target.value)); setDbdResult(null); setDbdError(""); }}
                  ...
                  {taxId && <span className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs font-mono font-semibold ${taxId.length === 13 ? "text-emerald-600" : "text-slate-400"}`}>{taxId.length}/13</span>}
                </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          ...
          {type === "PERSONAL" && (
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-700 flex items-center gap-1.5"><Hash className="w-3.5 h-3.5 text-slate-400" />Tax ID (เลขประจำตัวประชาชน)</label>
              <div className="relative">
                <input value={taxId} onChange={(e) => setTaxId(formatTaxId(e.target.value))} inputMode="numeric" maxLength={13} placeholder="0000000000000"
                  className={`w-full border-2 rounded-xl px-4 py-2.5 text-sm font-mono focus:outline-none transition-colors ${
                    taxId && taxIdError ? "border-red-400 bg-red-50" :
                    taxId && !taxIdError ? "border-emerald-400 bg-emerald-50" :
                    "border-slate-200 focus:border-violet-500"
